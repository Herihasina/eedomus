...
function build_widgets(panel_id)
{
  ...
  else
  {
    ...
    panel_page_id = panel.panel_type_id;
...
$(document).on('pagebeforeshow','#widgets_page',function(){
  ...
  $(document).on('tap','#widgets_list > li > a.ui-icon-gear',function(){ 
    ...
    alert(typeof $(this).data('widget-id'));
    alert(widgets);
    widgets_Page_ID = $(this).data('widget-id');
    //alert(widgets_Page_ID[0].type);
    alert(widgets);
    //alert(widgets_Page_ID.widgets.length);
...
// Programmation                typeof widgets_Page_ID == 'undefined' / null
// panel_page_id est 6 pour programmation recuperer à la ligne 960

/*if(typeof widgets_Page_ID == 'undefined'){

  if(widgets.controller_wizard_id > 0){
    // traitement pour programmation

  }
  else if(widgets.controller_module_id > 0)
  {*/
      ...
      $.ajax({
        url: 'config_select.php',
            ...
            success: function(response){
              nom = response.custom_name;response
              alert("reponse");
              alert(response[0]);
              $.ajax({
